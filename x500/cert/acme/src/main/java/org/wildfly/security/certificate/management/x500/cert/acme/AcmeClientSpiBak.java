...
//import static org.wildfly.security.certificate.management.x500.cert.acme.Acme.AUTHORIZATION;
...
//import static org.wildfly.security.certificate.management.x500.cert.acme.Acme.CAA_IDENTITIES;
...
//import static org.wildfly.security.certificate.management.x500.cert.acme.Acme.HEAD;
...
//import static org.wildfly.security.certificate.management.x500.cert.acme.Acme.WEBSITE;
...
// * @since 1.5.0
// */
//public abstract class AcmeClientSpiBak {
//
//    /**
...
//        Uni<HttpResponse<String>> connection = null;
...
//            }
//            connection = sendGetRequest(account.getServerUrl(staging), HttpURLConnection.HTTP_OK, JSON_CONTENT_TYPE);
...
//        }
//        connection
////               .future(httpClient.sendAsync(null, null))
//                .onItem().transform((HttpResponse<String> httpResponse) -> {
...
//                        } catch (MalformedURLException e) {
//                            return Uni.createFrom().failure(acme.unableToRetrieveAcmeServerDirectoryUrls(e));
//                        }
//                        resourceUrls.put(resource, url);
//                    }
//                    return (Map<AcmeResource, URL>) resourceUrls;
//                    // return Uni.createFrom().item((Map<AcmeResource, URL>) resourceUrls);
//                });
...
//     * the ACME server
//     */
//    public Uni<Boolean> createAccount(AcmeAccount account, boolean staging) throws AcmeException {
...
//     */
//    public Uni<Boolean> createAccount(AcmeAccount account, boolean staging, boolean onlyReturnExisting) throws AcmeException {
//        Assert.checkNotNullParam("account", account);
...
//        }
//
//        Uni<HttpResponse<String>> connection = sendPostRequestWithRetries(account, staging, newAccountUrl, true,
//                getEncodedJson(payloadBuilder.build()), HttpURLConnection.HTTP_CREATED, HttpURLConnection.HTTP_OK);
//
//        return connection.onItem().transform((HttpResponse httpResponse) -> {
//            try {
//                account.setAccountUrl(getLocation(httpResponse.headers().firstValue("Location").toString(), ACCOUNT));
//            } catch (AcmeException e) {
//                return Uni.createFrom().failure(e);
//            }
//            return httpResponse.statusCode() == HttpURLConnection.HTTP_CREATED;
//        }).onItem().castTo(Boolean.class);
//    }
//
////    /**
...
//     * to obtain a certificate. https://datatracker.ietf.org/doc/html/rfc9444#section-4.2
//     *
//     * @param account the ACME account information to use (must not be {@code null})
//     * @param staging whether or not the staging server URL should be used
...
//        Assert.checkNotNullParam("account", account);
//        final Uni<URL> newNonceUrl = getResourceUrl(account, AcmeResource.NEW_NONCE, staging);
//        HttpClient httpClient = HttpClient.newHttpClient();
//        HttpRequest httpRequest;
//        try {
//            httpRequest = HttpRequest.newBuilder().method("HEAD", HttpRequest.BodyPublishers.noBody())
//                    .uri(newNonceUrl.await().indefinitely().toURI())
...
//                .onItem().transform((HttpResponse httpResponse) -> {
...
//        final java.net.URL[] resourceUrl = new URL[1];
//        getResourceUrls(account, staging).onItem().transform((Map<AcmeResource, URL> e) -> {
//            resourceUrl[0] = e.get(resource);
//            if (resourceUrl[0] == null) {
//                return Uni.createFrom().failure(acme.resourceNotSupportedByAcmeServer(resource.getValue()));
//            }
//            return resourceUrl[0];
//        });
//        return Uni.createFrom().item(resourceUrl[0]);
//    }
//
...
//                    .header(USER_AGENT, USER_AGENT_STRING).build();
//            return Uni.createFrom().future(httpClient.sendAsync(httpRequest, HttpResponse.BodyHandlers.ofString()))
//                    .onItem().call((HttpResponse httpResponse) -> {
//                        try {
//                            int responseCode = httpResponse.statusCode();
//                            if (responseCode != expectedResponseCode) {
//                                handleAcmeErrorResponse(httpResponse, responseCode);
//                            }
//                            String contentType = httpResponse.headers().firstValue("Content-Type").toString();
...
////    }
////
//    private Uni<HttpResponse<String>> sendPostRequestWithRetries(AcmeAccount account, boolean staging, String resourceUrl, boolean useJwk, String encodedPayload,
...
//                try {
//                    httpRequest = HttpRequest.newBuilder().POST(HttpRequest.BodyPublishers.ofString(jws.toString())).uri(new URI(resourceUrl))
//                            .header(ACCEPT_LANGUAGE, Locale.getDefault().toLanguageTag())
//                            .header(CONTENT_TYPE, JOSE_JSON_CONTENT_TYPE)
//                            .header(USER_AGENT, USER_AGENT_STRING).build();
//                } catch (URISyntaxException e) {
...
//            }
//            return Uni.createFrom().future(httpClient.sendAsync(httpRequest, HttpResponse.BodyHandlers.ofString()))
//                    .onItem().call((HttpResponse httpResponse1) -> {
//                        int responseCode = httpResponse1.statusCode();
//                        try {
//                            account.setNonce(getReplayNonce(httpResponse1)); // update the account nonce
//                            for (int expectedResponseCode : expectedResponseCodes) {
//                                if (expectedResponseCode == responseCode) {
//                                    if (expectedContentType != null) {
//                                        String contentType = httpResponse1.headers().firstValue("Content-Type").toString();
//                                        if (!checkContentType(contentType, expectedContentType)) {
//                                            return Uni.createFrom().failure(acme.unexpectedContentTypeFromAcmeServer(contentType));
//                                        }
//                                    }
//                                }
//                                return Uni.createFrom().item(httpResponse1);
//                            }
//                            handleAcmeErrorResponse(httpResponse1, responseCode);
//                        } catch (AcmeException e) {
//                            return Uni.createFrom().failure(e);
//                        }
//                        return Uni.createFrom().failure(acme.badAcmeNonce());
//                    });
//        } catch (Exception e) {
...
//        String nonce = connection.headers().firstValue(REPLAY_NONCE).toString();
//        if (nonce == null) {
//            return null;
//        }
...
//    }
//
//    private static String getLocation(String location, String urlType) throws AcmeException {
//        if (location == null) {
...
//    }
//
//    private static long getRetryAfter(HttpResponse connection, boolean useDefaultIfHeaderNotPresent) throws AcmeException {
//        long retryAfterMilli  = -1;
//        String retryAfter = connection.headers().firstValue(RETRY_AFTER).toString();
//        if (retryAfter != null) {
//            try {
//                retryAfterMilli = Integer.parseInt(retryAfter) * 1000;
//            } catch (NumberFormatException e) {
//                long retryAfterDate = Long.parseLong(connection.headers().firstValue(RETRY_AFTER).toString());
//                if (retryAfterDate != 0) {
...
//        return checkContentType(contentType, expectedMediaType);
//    }
//
...
////            }
////            extracted(connection, responseCode, responseMessage);
...
////        }
////    }
//    private static void handleAcmeErrorResponse(HttpResponse connection, int responseCode) throws AcmeException {
//        try {
//            String responseMessage = connection.body().toString();
//            if (! checkContentType(connection.headers().firstValue("Content-Type").get() , PROBLEM_JSON_CONTENT_TYPE)) {
//                throw acme.unexpectedResponseCodeFromAcmeServer(responseCode, responseMessage);
//            }
//            JsonObject jsonResponse = getJsonResponse(connection);
...
//        if (nonce == null) {
//            nonce = getNewNonce(account, staging).await().indefinitely();
//        }
//        return nonce;
//    }
//
...
//    }
//
//    private static boolean checkContentType(String contentType, String expectedMediaType) throws AcmeException {
//        if (contentType == null) {
//            return false;
//        }
